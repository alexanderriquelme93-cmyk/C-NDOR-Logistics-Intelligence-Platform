...
let viaStats = {};      // { via: { costPerKg, costPerM3, slaDias, n } }
let viaList = [];
let origenList = [];
let destinoList = [];
let usdClp = null;
...
async function init() {
  ...
  origenList = uniqueSorted(raw.map(r => (r.paisOrigen || '').trim()));
  destinoList = uniqueSorted(raw.map(r => (r.puertoDestino || '').trim()));
...
/* ---------- Lista de bultos ---------- */
function addBulto() {
  bultos.push({ id: nextId++, via: viaList[0] || '', origen: origenList[0] || '', destino: destinoList[0] || '', peso: '', L: '', W: '', H: '', vol: '', cant: 1 });
  renderBultos();
}
...
function rowFor(b) {
  ...
  const sel = selectInput(viaList, b.via, v => { b.via = v; recompute(); });
  tdVia.appendChild(sel);

  // Origen
  const tdOrigen = td();
  tdOrigen.appendChild(selectInput(origenList, b.origen, v => { b.origen = v; }));

  // Destino
  const tdDestino = td();
  tdDestino.appendChild(selectInput(destinoList, b.destino, v => { b.destino = v; }));
