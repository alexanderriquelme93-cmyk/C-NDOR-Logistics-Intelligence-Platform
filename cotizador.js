...
/* ---------- Lista de bultos ---------- */
function addBulto() {
  bultos.push({ id: nextId++, via: viaList[0] || '', peso: '', L: '', W: '', H: '', vol: '', cant: 1 });
  renderBultos();
}
...
function rowFor(b) {
  ...
  viaList.forEach(v => {
    ...
    if (v === b.via) o.selected = true;
    sel.appendChild(o);
  });
  sel.addEventListener('change', e => { b.via = e.target.value; recompute(); });
  ...
  const li = numInput(b.L, 'L', v => { b.L = v; recompute(); });
  const wi = numInput(b.W, 'A', v => { b.W = v; recompute(); });
  const hi = numInput(b.H, 'H', v => { b.H = v; recompute(); });
  wrap.append(li, sep('×'), wi, sep('×'), hi);
  tdDims.appendChild(wrap);

  // Volumen directo
  const tdVol = td('num');
  tdVol.appendChild(numInput(b.vol, '0', v => { b.vol = v; recompute(); }));
  ...
  tr.append(tdVia, tdPeso, tdDims, tdVol, tdCant, tdCost, tdTransit, tdDel);
  return tr;
}
